...
const HtmlWebpackPlugin    = require('html-webpack-plugin'); //It will inject bundle generated by webpack to index.html.
...
const config = {
  ...
  output: {
    ...
    filename: 'bundle.js',
    publicPath //url-loader can use this path to find big image files that are compiled out of bundle.js to build folder. make sure to put '/' ('../' does not work as webpac-dev-server does not create bundle.js and style.css in build folder) before and after the folder name 'build', otherwise, image link will be like 'build0123223223ds2.jpg', so the link tag in index.html does not work.
  },
  module: {
    rules: [
      {
